...
def _find_matching_roles(domains: list, specs: list, seniority: str, user_skills: list = None) -> list:
    """Find matching roles from the ontology with genuinely varied fit scores."""
    ...
    # Build a map of which specs the user explicitly chose vs which are just in the domain
    user_specs_lower = {s.lower() for s in specs} if specs else set()
    ...
    for cluster_name, specializations in CAREER_ONTOLOGY.items():
        ...
        # How relevant is this domain? First domain = primary, second = secondary
        domain_rank = 0
        ...
        for spec_name, spec_roles in specializations.items():
            ...
            # Only include roles from matching specializations (or all if no spec chosen)
            if not spec_match and specs:
                continue
                
            for role_idx, role in enumerate(spec_roles[:3]):
                # ── Deterministic scoring with real variation ──
                
                # Base: 60% for being in a matched domain
                score = 0.60
                
                # +15% if this is the user's PRIMARY domain (first selected)
                # +8% if secondary domain
                if domain_rank == 0:
                    score += 0.15
                else:
                    score += 0.08
                
                # +10% if the specialization was explicitly selected by the user
                if spec_match:
                    score += 0.10
                
                # +5% for each user skill that appears in the role title (max +10%)
                skill_hits = sum(1 for s in user_skills_lower 
                               if s in role.lower() or role.lower() in s)
                score += min(0.10, skill_hits * 0.05)
                
                # -4% for each position within the specialization (1st role = best, 3rd = worst)
                score -= role_idx * 0.04
                
                # -3% for secondary domain roles
                if domain_rank > 0:
                    score -= 0.03
                
                # +3% bonus if the user has many skills (5+) — shows breadth
                if len(user_skills) >= 5:
                    score += 0.03
                
                fit_score = round(min(0.97, max(0.55, score)), 2)
                
                # Dynamic reasoning based on actual score
                if fit_score >= 0.90:
